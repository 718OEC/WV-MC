...
let activeCampus = 'WV';
// Hardcoded Form URL for the "Start a Club" button
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // 1. Initialize System-Only Theme
  initSystemTheme();

  // 2. Wire up the Start Club Button
  const startClubBtn = document.getElementById("start-club-btn");
  if (startClubBtn) {
      startClubBtn.href = wvClubFormUrl;
  }
  
  // 3. Wire up the Search Bar & Grid
...
function initSystemTheme() {
  ...
  // Actively listen for the user's OS changing themes
...
function filterClubs() {
  ...
  // Safety check: Prevents a crash if allclubsdata.js fails to load
...
}  // 2. Wire up the Start Club Button (If it exists on this page)
  const startClubBtn = document.getElementById("start-club-btn");
  if (startClubBtn) {
      startClubBtn.href = wvClubFormUrl;
  }
  
  // 3. Wire up the Search Bar & Grid (If they exist on this page)
  const searchInput = document.getElementById("search-input");
  const clubGrid = document.getElementById("club-grid");
...
function filterClubs() {
  const searchInput = document.getElementById("search-input");
  // Safety check: if there's no search bar on this page, abort function
  if (!searchInput) return; 
